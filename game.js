class Game {
    ...
    preload(){
        this.backgroundImage = [
            { src: loadImage('./assets/background/NebulaAqua-Pink.png'), x: 0, speed: 1,},
            { src: loadImage('./assets/background/StarsSmall_1.png'), x: 0, speed: 1,},
            { src: loadImage('./assets/background/StarsSmall_2.png'), x: 0, speed: 2,},
            { src: loadImage('./assets/background/Stars-Big_1_1_PC.png'), x: 0, speed:3,},
        ]

        this.player.preload()
        this.obstacleDynamic.image = loadImage('./assets/obstacal/obstacalDynamic.png'), 
        this.rainbowImage = loadImage('./assets/coins/rainbow-cloud-icon.png')
        this.rainbowImageNextLevel = loadImage('./assets/newLevel/4BiS.gif')
    ...
    //  if level 1 --> execute this 

        if(this.level >= 1 ) {
            // if (frameCount % 100 === 0 ){
            
            // let newObstacleDynamic = new ObstacleDynamic(this.obstacleDynamic.image,this.speedLevelObstacle)
		...
		}    // --> increasing the amount of frames per level by 40 
        // else {
        //     if (frameCount % 20 === 0 ){

        //     let newObstacleDynamic = new ObstacleDynamic(this.obstacleDynamic.image,this.speedLevelObstacle, this.obstacleInterval)
    ...
    this.obstacleDynamicImageArray = this.obstacleDynamicImageArray.filter(obstacle => {
